## Notes from Telcon on 9 September 2021
...
### Upcoming Force2021 conference (virtual, December 7-9)

There was agreement in the call that we should schedule a hackathon, two half days before and after
the main conference (so December 6 and 10). There is interest to together not only work on code,
but also other joint activities. Martin will coordinate planning. For the conference itself, there is
little time to talk about topics relevant for the Software Citation Implementation WG.

### Starting an Institutions Task Force in the working group

No news this month, will reschedule for next call.

### Updates from Task Forces

- No meeting of Journals Task Force in August.
- Registries Task Force is in the process of starting a new community of practice.
- CodeMeta Task Force ran into a blocker: making final decisions about schema 3.0 (we had a few controversial discussions that could not easily be resolved). Task Force is now waiting for resolution by CodeMeta co-chairs Matt and Carl.
...
- [August 3, 2021](https://github.com/force11/force11-sciwg/blob/master/meetings/20210803-Notes.md)
